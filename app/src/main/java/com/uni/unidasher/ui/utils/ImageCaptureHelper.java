 ...
 * Usage:<br>
 * <ol>
 * <li>Launch camera app by calling {@link #launchCameraApp(int, android.app.Activity, String)}, save resulting Uri, handle (or ignore) exceptions.</li>
 * <li>In your onActivityResult(int requestCode, int resultCode, Intent data) call {@link #retrievePhotoResult(android.content.Context, String) ) }. You may want to inform user if photo cannot be read despite the result is RESULT_OK.</li>
 * </ol>
 * @author koral--
 *
 */
public class ImageCaptureHelper
{
    ...
    static
    {
        CONTENT_VALUES.put( MediaStore.Images.ImageColumns.DESCRIPTION, DESCRIPTION);
    }
    /**
...
// no-op
     ...
     * @param caller caller Activity
     * @param photoKey key in Shared Preferences for taken image, can be null
     * @throws android.content.ActivityNotFoundException if no camera app was found
     ...
     */
    public static void launchCameraApp ( int requestCode, Activity caller, String photoKey ) throws Exception
    {
        if ( photoKey == null )
        {
            photoKey = PHOTO_URI;
    ...
    public static void launchCameraApp ( int requestCode, Fragment caller, String photoKey ) throws Exception
    {
        if ( photoKey == null )
        {
            photoKey = PHOTO_URI;
        ...
        if (activity != null) {
            ContentResolver cr = caller.getActivity().getContentResolver();
            Uri takenPhotoUri = cr.insert( MediaStore.Images.Media.EXTERNAL_CONTENT_URI, CONTENT_VALUES );
            if ( takenPhotoUri == null )
            {
                throw new IllegalStateException( "Photo insertion failed" );
            ...
            SharedPreferences prefs = caller.getActivity().getSharedPreferences( PHOTO_SHARED_PREFS_NAME, Context.MODE_PRIVATE );
